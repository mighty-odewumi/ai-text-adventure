...
app.post('/generate', async (req, res) => {
    ...
    const fullPrompt = `You are the narrator of a text adventure game called '${gameTitle}'. ${gameDescription}
        ...
        Based on the player's action, describe the next scene. The scene should be captivating.
        ...
        Offer the player clear choices for what to do next, but do not explicitly list these options as a numbered list. Instead, weave the choices into the description of the scene.
        Make each scene be unique and interesting.
        `;
...
app.get('/start', async (req, res) => {
    ...
    const initialPrompt = `You are the narrator of a text adventure game called '${gameTitle}'. ${gameDescription}
        Describe the very first scene the player encounters. The scene should be captivating and include a few points of interest.
        Incorporate opportunities to find items ([Item: item name]), lose or gain health ([+10 Health] or [-5 Health]), earn score ([+5 Score]), and uncover secrets ([Secret: secret text]).
        `;
    try {
